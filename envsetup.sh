...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cm_") ; then
       STATIC_BUILD=$(echo -n $1 | sed -e 's/^cm_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $STATIC_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       STATIC_BUILD=
    fi
    export STATIC_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${STATIC_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the STATIC model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Static github
        T=$(gettop)
        pushd $T > /dev/null
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $STATIC_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/static/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
